...
​      还有一种可能是在生成kubectl.kubeconfig的时候，没有将环境变量${KUBE_APISERVER}替换掉，检查       下/opt/k8s/work/kubectl.kubeconfig配置文件中server配置项，如果不存在此配置也回报以上错误。

```
省略...    
    server: https://10.1.80.77:8443
  name: kubernetes
contexts:
- context:
    cluster: kubernetes
