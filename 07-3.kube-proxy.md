...
for node_name in ${!ALL_NODES[@]}
  do 
    echo ">>> $node_name"
    sed -e "s/##NODE_NAME##/$node_name/" -e "s/##NODE_IP##/${ALL_NODES[$node_name]}/" kube-proxy-config.yaml.template > kube-proxy-config-$node_name.yaml.template
    scp kube-proxy-config-$node_name.yaml.template root@${ALL_NODES[$node_name]}:/etc/kubernetes/kube-proxy-config.yaml
  done
```
...
tcp        0      0 10.1.80.71:10256        0.0.0.0:*               LISTEN      5659/kube-proxy
tcp        0      0 10.1.80.71:10249        0.0.0.0:*               LISTEN      5659/kube-proxy
```
...
for node_ip in ${ALL_NODES[@]}
  do
    echo ">>> ${node_ip}"
    ssh root@${node_ip} "`which ipvsadm` -ln"
  done
```
...
TCP  10.254.0.1:443 rr
  -> 10.1.80.71:6443              Masq    1      0          0
  -> 10.1.80.72:6443              Masq    1      0          0
  -> 10.1.80.73:6443              Masq    1      0          0
>>> 10.1.80.73
IP Virtual Server version 1.2.1 (size=4096)
Prot LocalAddress:Port Scheduler Flags
  -> RemoteAddress:Port           Forward Weight ActiveConn InActConn
