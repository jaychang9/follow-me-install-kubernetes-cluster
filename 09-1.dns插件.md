...
# 更换镜像地址
sed -i "s/k8s.gcr.io/coredns/g" coredns.yaml
```
...
pod/coredns-8854569d4-xmwkh   1/1     Running   0          33s
...
replicaset.apps/coredns-8854569d4   1         1         1       33s
```
...
$ kubectl get svc --all-namespaces |grep my-nginx
default       my-nginx     ClusterIP   10.254.228.6   <none>        80/TCP                   44s
```

创建另一个 Pod，查看 `/etc/resolv.conf` 是否包含 `kubelet` 配置的 `--cluster-dns` 和 `--cluster-domain`，是否能够将服务 `my-nginx` 解析到上面显示的 Cluster IP `10.254.228.6`
...
NAME                READY   STATUS    RESTARTS   AGE
dnsutils-ds-72vxb   1/1     Running   0          94s
dnsutils-ds-mxfgw   1/1     Running   0          94s
dnsutils-ds-wkxwf   1/1     Running   0          94s
dnsutils-ds-x2whc   1/1     Running   0          94s
```

``` bash
$ kubectl -it exec dnsutils-ds-72vxb  bash
root@dnsutils-ds-72vxb:/# cat /etc/resolv.conf
...
``` bash
$ kubectl exec dnsutils-ds-72vxb nslookup kubernetes
Server:         10.254.0.2
...
``` bash
$ kubectl exec dnsutils-ds-72vxb nslookup www.baidu.com
Server:         10.254.0.2
Address:        10.254.0.2#53
...
``` bash
$ kubectl exec dnsutils-ds-72vxb nslookup my-nginx

Server:         10.254.0.2
Address:        10.254.0.2#53

Name:   my-nginx.default.svc.cluster.local
Address: 10.254.228.6
```

``` bash
$ kubectl exec dnsutils-ds-72vxb nslookup kube-dns.kube-system.svc.cluster
Server:         10.254.0.2
Address:        10.254.0.2#53
...
``` bash
$ kubectl exec dnsutils-ds-72vxb nslookup kube-dns.kube-system.svc
Server:         10.254.0.2
Address:        10.254.0.2#53
...
``` bash
$ kubectl exec dnsutils-ds-72vxb nslookup kube-dns.kube-system.svc.cluster.local
Server:         10.254.0.2
Address:        10.254.0.2#53
