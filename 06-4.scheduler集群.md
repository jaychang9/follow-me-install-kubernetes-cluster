...
for node_ip in ${MASTER_NODES[@]}
  do
    sed -e "s/##NODE_IP##/$node_ip/" kube-scheduler.yaml.template > kube-scheduler-$node_ip.yaml
  done
ls kube-scheduler*.yaml
```
+ MASTER_NODES为类似map结构的shell关联数组,key为主机名，value为ip地址；
...
for node_ip in ${MASTER_NODES[@]}
  do
    sed -e "s/##NODE_IP##/$node_ip/" kube-scheduler.service.template > kube-scheduler-$node_ip.service 
  done
ls kube-scheduler*.service
```
+ MASTER_NODES为类似map的shell关联数组，key为主机名，value为ip地址；
...
tcp        0      0 10.1.80.71:10251        0.0.0.0:*               LISTEN      11286/kube-schedule
tcp        0      0 10.1.80.71:10259        0.0.0.0:*               LISTEN      11286/kube-schedule
```

``` bash
$ curl -s http://10.1.80.71:10251/metrics |head
...
``` bash
$ curl -s --cacert /opt/k8s/work/ca.pem --cert /opt/k8s/work/admin.pem --key /opt/k8s/work/admin-key.pem https://10.1.80.71:10259/metrics |head
...
metadata:
  annotations:
    control-plane.alpha.kubernetes.io/leader: '{"holderIdentity":"k8s-m2_4cb9ef92-9ba4-11e9-97f6-080027a05fda","leaseDurationSeconds":15,"acquireTime":"2019-07-01T02:02:38Z","renewTime":"2019-07-01T02:04:44Z","leaderTransitions":0}'
  creationTimestamp: "2019-07-01T02:02:38Z"
  name: kube-scheduler
  namespace: kube-system
  resourceVersion: "3300"
  selfLink: /api/v1/namespaces/kube-system/endpoints/kube-scheduler
  uid: 4d57afa4-9ba4-11e9-9e97-080027a05fda
```

可见，当前的 leader 为 k8s-m2 节点。
