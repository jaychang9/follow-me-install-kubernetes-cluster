...
listen kube-master
    ...
    option tcplog
    balance source
    server 10.1.11.119 10.1.11.119:6443 check inter 2000 fall 2 rise 2 weight 1
    server 10.1.11.120 10.1.11.120:6443 check inter 2000 fall 2 rise 2 weight 1
    server 10.1.11.125 10.1.11.125:6443 check inter 2000 fall 2 rise 2 weight 1
EOF
```
...
``` bash
scp keepalived-master.conf root@10.1.11.119:/etc/keepalived/keepalived.conf
```

下发 backup 配置文件：

``` bash
scp keepalived-backup.conf root@10.1.11.120:/etc/keepalived/keepalived.conf
scp keepalived-backup.conf root@10.1.11.125:/etc/keepalived/keepalived.conf
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh ${node_ip} "/sbin/ip addr show ${VIP_IF}"
