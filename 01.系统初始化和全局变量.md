...
ssh-copy-id root@10.1.80.71
ssh-copy-id root@10.1.80.72
ssh-copy-id root@10.1.80.73
ssh-copy-id root@10.1.80.74
```
...
environment.sh在manifests目录下，先将改文件上传到/opt/k8s/bin
...
for node_name in ${!ALL_NODES[@]};do 
   ...
   ssh root@${node_name} hostnamectl set-hostname ${node_name}
done
```
...
for node_name in ${!ALL_NODES[@]}; do
   echo ">>> ${node_name} ${ALL_NODES[${node_name}]}"
   ssh root@${node_name} useradd -m docker
done
```
...
for node_name in ${!ALL_NODES[@]}; do
echo ">>> ${node_name} ${ALL_NODES[$node_name]}"
cat << EOF | ssh root@${node_name} 'cat - >> /root/.bashrc'
`cat path.txt`
EOF
done
...
for node_name in ${!ALL_NODES[@]}; do
echo ">>> ${node_name} ${ALL_NODES[$node_name]}"
ssh root@$node_name source /root/.bashrc
done
```
...
for node_name in ${!ALL_NODES[@]}; do
 ...
 sed -i 's#http://xxx.ubuntu.com#https://mirrirs.aliyun.com#g' /etc/apt/sources.list
 sed -i 's#http://yyy.ubuntu.com#https://mirrirs.aliyun.com#g' /etc/apt/sources.list
 apt update -y
done
```
...
for node_name in ${!ALL_NODES[@]}; do
  echo $node_name ${ALL_NODES[$node_name]}
  ssh root@$node_name apt-get install -y conntrack ipvsadm ntp ipset jq iptables curl sysstat libseccomp2
done
```
...
for node_name in ${!ALL_NODES[@]}; do
    echo ">>> $node_name ${ALL_NODES[$node_name]}"
    ssh root@$node_name setenforce 0
    ssh root@$node_name sed -i 's/^SELINUX=.*/SELINUX=disabled/' /etc/selinux/config
done
```
...
for node_name in ${!ALL_NODES[@]}; do
    echo ">>> $node_name ${ALL_NODES[$node_name]}"
	ssh root@$node_name systemctl stop dnsmasq && systemctl disable dnsmasq
done	
```
...
for node_name in ${!ALL_NODES[@]}; do
    echo ">>> $node_name ${ALL_NODES[$node_name]}"
	ssh root@$node_name modprobe ip_vs_rr && modprobe br_netfilter
done
```
...
for node_name in ${!ALL_NODES[@]}; do
	echo ">>> $node_name ${ALL_NODES[$node_name]}"
	scp kubernetes.conf  root@$node_name:/etc/sysctl.d/kubernetes.conf
	ssh root@$node_name sysctl -p /etc/sysctl.d/kubernetes.conf
done	
```
...
for node_name in ${!ALL_NODES[@]}; do
    ...
    # 调整系统 TimeZone
    ssh root@$node_name timedatectl set-timezone Asia/Shanghai

    # 将当前的 UTC 时间写入硬件时钟
    ssh root@$node_name timedatectl set-local-rtc 0

    # 重启依赖于系统时间的服务
    ssh root@$node_name systemctl restart rsyslog 
    ssh root@$node_name systemctl restart crond
done   
```
...
for node_name in ${!ALL_NODES[@]}; do
    echo ">>> $node_name ${ALL_NODES[$node_name]}"
	ssh root@$node_name systemctl stop postfix && systemctl disable postfix
done	
```
...
for node_name in ${!ALL_NODES[@]}; do
    echo ">>> $node_name ${ALL_NODES[$node_name]}"
    scp 99-prophet.conf root@$node_name:/etc/systemd/journald.conf.d/99-prophet.conf
    ssh root@$node_name mkdir /var/log/journal # 持久化保存日志的目录
    ssh root@$node_name mkdir /etc/systemd/journald.conf.d
    ssh root@$node_name systemctl restart systemd-journald
done
```
...
for node_name in ${!ALL_NODES[@]}; do
    echo ">>> $node_name ${ALL_NODES[$node_name]}"
	ssh root@$node_name mkdir -p  /opt/k8s/{bin,work} /etc/{kubernetes,etcd}/cert
done	
```
...
for node_name in ${!OTHER_NODES[@]}; do
echo ">>> $node_name ${OTHER_NODES[$node_name]}"
scp linux*.deb root@$node_name:/opt/k8s/work
done
...
for node_name in ${!ALL_NODES[@]}; do
echo ">>> $node_name ${ALL_NODES[$node_name]}"
ssh root@$node_name dpkg -i /opt/k8s/work/linux*.deb
done
```
...
for node_name in ${!ALL_NODES[@]}; do
echo ">>> $node_name ${ALL_NODES[$node_name]}"
ssh root@$node_name apt purge linux*4.15.0* -y
done
```
