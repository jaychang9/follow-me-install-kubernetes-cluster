...
cd /opt/k8s/work
source /opt/k8s/bin/environment.sh
touch hosts.txt && echo "" > hosts.txt
for node_name in ${!ALL_NODES[@]}; do 
  echo $node_name ${ALL_NODES[$node_name]} >> hosts.txt
done

# 批量设置hosts
for node_name in ${OTHER_NODES[@]}; do 
echo ">>> ${node_name} "
cat << EOF | ssh root@${node_name} 'cat - >> /etc/hosts'
   `cat hosts.txt`
EOF
done
```
...
for node_name in ${!ALL_NODES[@]};do
    echo ">>> ${node_name} ${ALL_NODES[${node_name}]}"
	ssh root@${node_name} echo 'PATH=/opt/k8s/bin:$PATH' >>/root/.bashrc
